...
def get_ai_explanation(features, predicted_price, api_key):
    ...
    try:
        ...
        if "insufficient_quota" in error_str or "429" in error_str or "quota" in error_str.lower():
            # Rule-based fallback (Manual Expert Logic)
            sqft = features.get('sqft_living', 0)
            grade = features.get('grade', 7)
            ...
            drivers = []
            if sqft > 2500:
                drivers.append("generous living space")
            elif sqft < 1200:
                drivers.append("compact, efficient layout")
                
            if grade >= 9:
                drivers.append("high-quality construction grade")
            elif grade <= 6:
                drivers.append("standard construction grade")
                
            driver_str = " and ".join(drivers) if drivers else "property characteristics"
            explanation = f"Price is primarily driven by the {driver_str}. At ${ppsf:,.0f}/sqft, valuation reflects "
            if ppsf > 400:
                explanation += "a premium market position."
            elif ppsf < 200:
                explanation += "a value-focused market segment."
            else:
                explanation += "standard market conditions."

            # 2. Generate Market Realism
            realism = "Realistic."
            if ppsf > 800:
                realism = "High Estimate: Price per sqft is significantly above average, typical only for luxury or prime locations."
            elif ppsf < 100:
                realism = "Low Estimate: Valuation seems conservative; ensure property condition isn't distressed."
            else:
                realism = f"The price of ${price:,.0f} appears consistent with local trends for {grade} grade properties of this size."

            # 3. Biases
            biases = "Model may not fully account for recent renovations, specific view corridors, or micro-neighborhood demand."
            if grade < 6:
                biases += " Lower grade properties often have wide variance due to condition."
