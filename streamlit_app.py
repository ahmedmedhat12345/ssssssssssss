    ...
    try:
        ...
        if "insufficient_quota" in error_str or "429" in error_str or "quota" in error_str.lower():
            ...
            if ppsf > TYPICAL_PPSF * 2:  # Double the average
                realism = "⚠️ High Estimate: Price per sqft significantly exceeds market average. Typical for luxury properties or prime locations."
            elif ppsf < TYPICAL_PPSF * 0.5:  # Half the average
                realism = "⚠️ Low Estimate: Valuation is well below market average. May indicate distressed property or data anomalies."
            elif price > MEAN_PRICE * 1.5:
                realism = f"Above Average: At ${price:,.0f}, this exceeds typical market price (${MEAN_PRICE:,.0f}) but remains realistic for larger/premium properties."
            elif price < MEAN_PRICE * 0.5:
                realism = f"Below Average: At ${price:,.0f}, this is below typical market price (${MEAN_PRICE:,.0f}), consistent with smaller or value properties."
            else:
                realism = f"Realistic: The price of ${price:,.0f} aligns closely with market trends for properties of this profile."

            # 3. Biases
            biases = "Model limitations: Does not account for recent renovations, specific views, micro-location premiums, or current market momentum. "
            if grade < MODE_GRADE - 1:
                biases += "Lower-grade properties show higher price variance due to condition variability. "
            if sqft > MEAN_SQFT * 2:
                biases += "Large homes may have non-linear pricing (luxury premiums not fully captured). "
            if sqft < MEAN_SQFT * 0.5:
                biases += "Smaller properties may reflect niche markets (condos, studios) with different pricing dynamics."

            return {
                "Explanation": f"**[Fallback Analysis - Data-Driven]** {explanation}",
                "MarketRealism": realism,
                "PotentialBiases": biases.strip()
            }
        return {"error": str(e)}
